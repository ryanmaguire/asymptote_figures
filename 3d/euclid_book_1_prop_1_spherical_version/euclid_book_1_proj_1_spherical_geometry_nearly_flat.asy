...
vec3.Vec3 A = vec3.FromSpherical(1.0, +pi/8, pi/2);
vec3.Vec3 B = vec3.FromSpherical(1.0, -pi/8, pi/2);
vec3.Vec3 U = vec3.Vec3(1.0, 0.0, 0.0);

vec2.Vec2 c1(real t)
{
    vec3.Vec3 P = vec3.SphericalCircle(t, 1.0, A, B);
    return vec3.OrthographicProjection(P, U).Rotate(-pi / 4);
}
...
guide q1 = pc.PathFromParametricFunction(c1, -pi, pi, 100, closed = true);
guide q2 = pc.PathFromParametricFunction(c2, -pi, pi, 100, closed = true);

vec2.Vec2 C1 = vec3.StereographicProjection(vec3.SphericalCircle(0, 1, A, B));
vec2.Vec2 C2 = vec3.StereographicProjection(vec3.SphericalCircle(1, 1, A, B));
vec2.Vec2 C3 = vec3.StereographicProjection(vec3.SphericalCircle(2, 1, A, B));

vec2.Vec2 D1 = vec3.StereographicProjection(vec3.SphericalCircle(0, 1, B, A));
vec2.Vec2 D2 = vec3.StereographicProjection(vec3.SphericalCircle(1, 1, B, A));
vec2.Vec2 D3 = vec3.StereographicProjection(vec3.SphericalCircle(2, 1, B, A));
